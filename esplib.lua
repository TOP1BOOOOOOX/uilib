...
local esp = {
    debug = false,  -- << SET THIS TO true IN YOUR SCRIPT FOR LOGS
...
function esp:update()
    for plr, drawing in next, esp.players do
        ...
        if not player then
            if esp.debug then print("[ESP] Player", plr, "left game, removing") end
            esp.players[plr] = nil
            continue
        end

        if not esp.enabled then
            if esp.debug then print("[ESP] Disabled, skipping all") end
            esp:disable(player)
            continue
        end

        if not esp.checkalive(player) then
            if esp.debug then print("[ESP]", plr, "not alive or no character") end
            esp:disable(player)
            continue
        end
        ...
        if not character or not character:FindFirstChild('HumanoidRootPart') then
            if esp.debug then print("[ESP]", plr, "character missing HumanoidRootPart") end
            esp:disable(player)
            continue
        end
        ...
        if esp.debug then
            print(string.format("[ESP] %s | pass=%s onScreen=%s dist=%s flag=%s",
                playerName, tostring(pass), tostring(onScreen), distance,
                esp.checkteam(player, false) and 'enemy_' or 'team_'
            ))
        end

        if not (pass and onScreen) then
            if esp.debug then print("[ESP] "..plr.." disabled (off screen or failed checks)") end
            esp:disable(player)
        end
        ...
        if drawing.arrow.Visible then
            ...
            -- (arrow info omitted for brevity, same as before)
        end
        ...
        -- If not pass or not onScreen, skip 2D box calculations
        if not pass or (not onScreen) then
            if esp.debug then print("[ESP] "..plr.." skipping 2D (pass="..tostring(pass).." onScreen="..tostring(onScreen)..")") end
            continue
        end

        if esp.debug then print("[ESP] "..plr.." calculating 2D box...") end

        -- check for missing body parts
        if not (character:FindFirstChild('Head') and character:FindFirstChild('RightHand') and character:FindFirstChild('LeftHand') and character:FindFirstChild('RightFoot') and character:FindFirstChild('LeftFoot')) then
            if esp.debug then print("[ESP] "..plr.." missing body parts for box offsets") end
            continue
        end
        ...
        if esp.debug then
            print(string.format("[ESP] %s box bounds: X[%.0f-%.0f] Y[%.0f-%.0f]", playerName, smallestX, biggestX, smallestY, biggestY))
        end

        -- if the box is degenerate, skip drawing
        if biggestX - smallestX <= 0 or biggestY - smallestY <= 0 then
            if esp.debug then print("[ESP] "..plr.." box size zero, skipping") end
            continue
        end
        ...
        if drawing.box.Visible then
            ...
            if esp.debug then print("[ESP] "..plr.." box drawn at", drawing.box.Position, drawing.box.Size) end
        end

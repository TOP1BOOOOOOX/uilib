--// ESP Library – Works even with non‑standard rigs & cameras
...
local esp = {
    ...
    arrowsize = 20,
    arrowinfo = false,
    alwaysShowBoxes = true,   -- show 2D ESP even when off‑screen (clamped to screen edges)
...
--// Helper to create Instance objects (for Highlights)
...
function esp:update()
    for name, drawing in next, self.players do
        ...
        local distance = FLOOR((centerMassPos.p - camera.CFrame.p).Magnitude / 3)
        local screenPos, onScreen = camera:WorldToViewportPoint(root.Position)

        -- Determine flag (team_ / enemy_ / priority_)
        local flag = self.checkteam(player) and 'enemy_' or 'team_'
        if TFIND(self.priority_players, player) then flag = 'priority_' end

        print(string.format("[ESP] %s | pass=%s onScreen=%s dist=%dm flag=%s",
            name, tostring(pass), tostring(onScreen), distance, flag))

        -- Disable everything first, then re-enable what should show
        for i, v in next, drawing do
            if i == 'chams' then v.ins.Enabled = false else v.Visible = false end
        end

        -- Chams (always show if pass)
        ...
        -- Arrows (show when pass, even off-screen)
        ...
        -- Box calculation (safe fallback for missing limbs)
        ...
        if head and rightHand and leftHand and rightFoot and leftFoot then
            ...
            -- fallback: simple cube based on head position
        ...
        -- If alwaysShowBoxes and off-screen, clamp to screen edges
        ...
        local topText = showName and (showDist and '['..distance..'m] '..playerName or playerName) or nil
        local distText = drawing.distance.Visible and '['..distance..'m]' or nil
